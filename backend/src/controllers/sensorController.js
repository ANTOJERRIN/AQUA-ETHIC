...
class SensorController {
    ...
    async receiveData(req, res) {
        try {
            const { deviceId, pH, temperature, turbidity, dissolvedOxygen } = req.body;
            ...
            const recordId = await databaseService.saveReading(
                deviceId, pH, temperature, turbidity, dissolvedOxygen, dataHash
            );
            ...
            if (tx) {
                await databaseService.updateBlockchainInfo(recordId, tx.txHash, tx.blockNumber);
    ...
    async getHistory(req, res) {
        try {
            ...
            const readings = await databaseService.getHistory(deviceId, parseInt(limit));
